...
OUTPUT_DIR = BASE_DIR / "data" / "backtests" / "trades"
...
def generate_trades(filtered_rows, vwap_rows):
    ...
    for row in filtered_rows:
        ...
        open_price = float(row["0915_open"])
        high = float(row["0915_high"])
        low = float(row["0915_low"])
        close = float(row["0915_close"])
        day_high = float(row["day_high"])
        day_low = float(row["day_low"])
        close_1518 = float(row["1518_close"])
        ...
        if day_low <= prev_vwap <= day_high:
            exit_price = prev_vwap
        else:
            exit_price = close_1518
